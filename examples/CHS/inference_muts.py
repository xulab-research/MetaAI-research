...
def main(mut_counts: int):
    ...
    with open("../../data/example_protein/CHS/features/wt/result.fasta", "r") as f:
    ...
    esm_path = "../../data/example_protein/CHS/features/wt/esmc_600m_embedding.pt" if FEATURE_NAME == "esmc_600m" else "../../data/example_protein/CHS/features/wt/esmc_300m_embedding.pt"
    esm = load_tensor_maybe_dict(esm_path).to(torch.float32).unsqueeze(0).to(device)
    spired = load_tensor_maybe_dict("../../data/example_protein/CHS/features/wt/spired_fitness_embedding.pt").to(torch.float32).unsqueeze(0).to(device)

    df = pd.read_csv(f"../../data/example_protein/CHS/candidates/sorted_mut_counts_{mut_counts}.csv")
